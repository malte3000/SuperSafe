...
const funds = [
  {
    ...
    company: 'Länsförsäkringar Fondförvaltning AB',
    isin: 'SE0005188836',
  },
  {
    id: 'vision',
    name: 'Länsförsäkringar Global Vision',
    company: 'Länsförsäkringar Fondförvaltning AB',
    isin: 'SE0000837205',
  },
  {
    id: 'zero',
    name: 'Avanza Zero',
    company: 'Avanza Fonder AB',
    isin: 'SE0001718388',
  },
  {
    id: 'alfred',
    name: 'Alfred Berg Sverige',
    company: 'Alfred Berg',
    isin: null,
  },
];
...
test('accents, decomposed Unicode, repeated spaces and punctuation normalize', () => {
  assert.equal(
    normalizeFundSearch('  LÄNSFÖRSÄKRINGAR  Global–Index '),
    'lansforsakringar global index',
  );
  assert.equal(
    normalizeFundSearch('La\u0308nsfo\u0308rsa\u0308kringar'),
    'lansforsakringar',
  );
  for (const query of [
    'lansforsakringar global index',
    '  LÄNSFÖRSÄKRINGAR   GLOBAL INDEX',
    'Index, Global Länsförsäkringar',
  ]) {
...
test('LF expands only as a whole word and works in either word order', () => {
  for (const query of ['LF Global', 'global lf', 'lf-global']) {
    assert.deepEqual(
      searchFiFunds(funds, query).map((f) => f.id),
      ['global', 'vision'],
    );
  }
  assert.deepEqual(
    searchFiFunds(funds, 'Alfred').map((f) => f.id),
    ['alfred'],
  );
});
test('common bank abbreviations and FI fund numbers are searchable', () => {
  const handelsbanken = {
    id: 'hb-global',
    name: 'Handelsbanken Global Index Criteria',
    company: 'Handelsbanken Fonder AB',
    isin: 'SE0000000001',
    instituteNumber: '61032',
  };
  assert.equal(
    searchFiFunds([...funds, handelsbanken], 'SHB global')[0].id,
    'hb-global',
  );
  assert.equal(
    searchFiFunds([...funds, handelsbanken], 'HB global')[0].id,
    'hb-global',
  );
  assert.equal(
    resolveFiFund([...funds, handelsbanken], '61032').id,
    'hb-global',
  );
  assert.equal(
    searchFiFunds([...funds, handelsbanken], '6103')[0].id,
    'hb-global',
  );
});
test('ISIN can contain formatting spaces and hyphens, without fuzzy code matching', () => {
  ...
  assert.deepEqual(
    searchFiFunds(funds, 'SE000518').map((f) => f.id),
    ['global'],
  );
...
test('ambiguous searches require selection while unique or exact searches resolve', () => {
  ...
  assert.equal(
    resolveFiFund(
      [...funds, { ...funds[0], id: 'duplicate-class' }],
      funds[0].name,
    ),
    undefined,
  );
...
test('spelling suggestions are conservative and never replace normal results', () => {
  assert.deepEqual(
    suggestFiFunds(funds, 'Avnaza Zero').map((f) => f.id),
    ['zero'],
  );
  assert.deepEqual(
    suggestFiFunds(funds, 'lansforsakringar gloabl index').map((f) => f.id),
    ['global'],
  );
  assert.deepEqual(suggestFiFunds(funds, 'Avanza Zero'), []);
  assert.deepEqual(suggestFiFunds(funds, 'az zaro'), []);
});
test('main search and comparison filter agree on real FI funds', async () => {
  const { funds: real } = JSON.parse(
    await readFile(
      new URL('../public/data/fi-funds-latest.json', import.meta.url),
      'utf8',
    ),
  );
  for (const query of [
    'LF Global',
    'global lansforsakringar',
    'avanza',
    'SE 0005-188836',
    'DNB Global Indeks',
  ]) {
    const main = searchFiFunds(real, query)
      .map((f) => f.id)
      .sort();
    const comparison = real
      .filter((f) => matchesFiFund(f, query))
      .map((f) => f.id)
      .sort();
  ...
  assert.ok(
    searchFiFunds(real, 'global index lf').some((f) => f.id === 'SE0005188836'),
  );
});
