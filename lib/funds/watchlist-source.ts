...
async function loadWatchlists(readOnly = false): Promise<WatchlistData> {
  ...
  let snapshot: WatchSnapshot & { sourceUnavailable?: boolean } = cached ? await cached.json<WatchSnapshot>() : baseline;
  ...
  if (!readOnly && (!cached || now.getTime() - Date.parse(snapshot.fetchedAt) > 6 * 3600000)) {
    try {
      ...
      await bucket.put('watchlist/latest.json', JSON.stringify({ ...snapshot, sourceUnavailable: true }), { httpMetadata: { contentType: 'application/json' } });
    }
  }
